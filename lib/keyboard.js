        ...
        this.addBtnRow = async (buttons) => {
            ...
            for (let button of buttons) {
                ...
                if (typeof button === 'string') {
                    button = this.translatable ? await this.BC.lexicon(button, {}, this._language, this.ctx) : button
                } else if (typeof button === 'object') {
                    if (!button.data) {
                        button.data = button.text
                    }
                    if (this.translatable) button.text = await this.BC.lexicon(button.text, button.params || {}, this._language, this.ctx)
                }
        ...
        this.fromKBObject = async (kbObject = {}) => {
            ...
            if (kbObject.method !== undefined) {
                ...
                if (method) {
                    ...
                    kbObject = this.BC.MT.mergeRecursive(await method(this.ctx, kbObject), kbObject);
                }
            }
