...
const MessageElements = require('./messageelements')
...
const AbstractDriver = require('./drivers/abstractdriver')
 ...
 * @property {boolean|string} [replace] Indicates if new messages must replace (edit) original
...
class BotCMS {
    ...
    constructor (config = {}) {
        ...
        /** @deprecated */
        this.ATTACHMENTS = TYPES.ATTACHMENTS;
        this.BINDINGS = {
            FILE: '$FILE ',
            METHOD: '$METHOD ',
        };
        ...
        this.defaults = {
            ...
            blockEvents: [EVENTS.BOT_BLOCKED_BY_USER, EVENTS.BOT_KICKED_FROM_CHAT, EVENTS.CHAT_NOT_FOUND,
                EVENTS.USER_NOT_FOUND, EVENTS.USER_BLOCKED],
            storeExcludeValues: []
        };
        ...
        this.Auth2FA = new this.config.classes.Auth2FA(this);
    ...
    async initNetworks (networks) {
        ...
        if (!this.MT.empty(networks)) {
            for (let network in networks) {
                ...
                promises.push((async () => this.initNetwork(networks[network]))())
            }
        }
        return Promise.all(promises)
    }

    async initNetwork (networkConfig) {
        let name = networkConfig['name'] ? networkConfig['name'] : network;
        let driver;
        let driverName = this.MT.empty(networkConfig['driver']) ? name : networkConfig['driver'];
        ...
        let tmp = new driver(this, networkConfig);
        if (!(tmp instanceof AbstractDriver)) console.error('BOTCMS DRIVER', name, 'IS NOT EXTENDS OF ABSTRACT DRIVER')
    ...
    async handleUpdate (ctx) {
        ...
        console.log('EVENT', ctx.Message.event, 'MESSAGE', ctx.Message.text);
    ...
    async doUpdate (current, ctx, opts = {}) {
        ...
        await this.store(current, ctx, true);
        let result = null;

        if (['forceNone', 'forceCurrent'].indexOf(current.replace) !== -1) opts.replace = current.replace

        await this.doAction(current, ctx);

        if ((!this.MT.empty(current.message) || !this.MT.empty(current.attachments)) && this.config.blockEvents.indexOf(ctx.Message.event) === -1) {
            let parcel = ctx.newParcel()
            ...
            parcel.keyboard = await (new this.config.classes.Keyboard(ctx))
              ...
              .then(kb => kb.build());
            if ('attachments' in current) await parcel.handleAttachments(current.attachments || {});
            let  editMsgId = 0
            if ((current.replace && opts.replace !== 'forceNone') || opts.replace === 'forceCurrent') {
                if (this.isFormOpen(current)) {
                    if (opts.replace === 'forceCurrent') editMsgId = ctx.Message.id
                    else if (opts.formMsgId) editMsgId = opts.formMsgId
    ...
    async doAction(step, ctx = undefined) {
        ...
        if (!this.MT.empty(step.action)) {
            ...
            const actions = this.MT.makeArray(step.action)
            for (let action of actions) {
                if (this.MT.isString(action)) {
                    action = { type: 'method', method: action };
                }
                let type = typeof action === 'function' ? 'method' : this.MT.extract('type', action, 'method')
                let parameters = this.MT.extract('params', action, this.MT.extract('options', action, {}));
                ...
                switch (type) {
                    ...
                    case 'method':
                        let path = action
                        if (typeof action === 'object') path = action.method || action.name || action.value
                        const method = typeof action === 'function' ? action : this.MT.extract(path)
                        if (method) {
                            promises.push(method(ctx, params));
                        } else {
                            console.error('BOTCMS DO ACTION. METHOD ' + (typeof action === 'object' ? action.method : action) + ' NOT FOUND');
                        }
                        break;
                }
            }
    ...
    async getStepByCommand (ctx) {
        ...
        for (let i in this.commands) {
            ...
            if (step !== undefined && step !== null && !this.MT.empty(step.trigger) && await this.T.checkTrigger(ctx, step.trigger)) {
    ...
    async findNextStep (actions, ctx, exclude = []) {
        let nextStep = {};
        if (!this.MT.empty(actions) && !this.MT.empty(actions.goto)) {
            ...
            if (!this.MT.empty(actions.goto)) {
                ...
                if (!this.MT.empty(next)) {
                    ...
                    console.error('NEXT STEP PATH ' + actions.goto + ' SPECIFIED AND NOT FOUND. CURRENT PATH: ' + ctx.session.step?.path);
                }
            } else {
     ...
     * @param {boolean} pre
     */
    async store (step, ctx, pre = false) {
        let store = (pre ? (step.storePre || step.store_pre) : step.store) || {};
        if (this.MT.empty(store)) {
            return;
        }
        for (const exclude of this.config.storeExcludeValues) {
            if (ctx.msg === await ctx.lexicon(exclude)) return
        }
        if (store === true) {
            store = {};
        }
    ...
    needDeleteFormAnswer (step) {
        return typeof step === 'object' && (typeof step.form === 'object' && step.form.question === true && step.form.deleteAnswer === true)
    }
    ...
    async formStore (ctx, step, ids = [], outboundIds = false) {
        ...
        if (isFormOpen) {
            ...
            if (typeof ctx.session.form !== 'object' || this.MT.empty(ctx.session.form)) {
                ...
                if (Array.isArray(ctx.session.form.formQuestionIds) && ctx.session.form.formQuestionIds.length) {
                    await ctx.remove(ctx.session.form.formQuestionIds, ctx.session.form.formChatId)
                    ctx.session.form.formQuestionIds = []
                } else ctx.session.form.formQuestionIds = []
                if (Array.isArray(ctx.session.form.formAnswerIds) && ctx.session.form.formAnswerIds.length) {
                    await ctx.remove(ctx.session.form.formAnswerIds, ctx.session.form.formChatId)
                    ctx.session.form.formAnswerIds = []
                } else ctx.session.form.formAnswerIds = []
                ...
                if (outboundIds) {
                    ...
                    for (let id of ids) {
                        if (ctx.session.form.formAnswerIds.indexOf(id) === -1 && ctx.session.form.formQuestionIds.indexOf(id) === -1) {
                            if (this.needDeleteFormAnswer(step)) await ctx.remove(id, ctx.session.form.formChatId)
                            else ctx.session.form.formAnswerIds.push(id)
                        }
                    }
                }
                // form = ctx.session.form
     ...
     */
    async lexicon (key, params = {}, language = undefined, ctx = undefined) {
    ...
    launchNetwork(bridge, middleware, ...middlewares) {
        return bridge.launch(middleware, ...middlewares).then(async () => {
            if (this.MT.extract('notifyLaunch.bridge', this.config) === bridge.name) {
                ...
                await this.bridgeExec({ bridge, method: 'send', params: parcel }).catch(e => console.error('ERROR WHILE NOTIFY ABOUT START:', e))
            }
        });
    }
    ...
    async bridgeExec (execParams, logParams = {}) {
        ...
        if (this.MT.empty(bridge)) return
        const bridgeName = (typeof execParams.bridge === 'string') ? execParams.bridge : bridge.name
        if (execParams.method in bridge) {
            execParams.params = this.MT.makeArray(execParams.params)
            const methodWrapper = async () => bridge[execParams.method](...execParams.params)
            const methodResult = await methodWrapper().catch((e) => {
                console.error('BRIDGE EXEC METHOD', execParams.method, 'FAILURE:', e)
                return { success: false, error: e.name + ' (' + e.description + ')' }
            })
            if (!execParams.noLog) {
                ...
                if ('additional' in execParams) logData.additional = execParams.additional
                if ('sid' in execParams) logData.sid = execParams.sid
    ...
    async requestCode2FA (bridge, params) {
        return this.Auth2FA.requestCode(bridge, params)
    }
}

BotCMS.EVENTS = EVENTS
BotCMS.TYPES = TYPES
...
BotCMS.MessageElements = MessageElements
BotCMS.Parcel = Parcel
BotCMS.Logger = Logger
BotCMS.Tools = Tools
BotCMS.MVTools = MVTools
BotCMS.AbstractDriver = AbstractDriver
