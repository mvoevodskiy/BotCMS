        ...
        if (!this.MT.empty(step.action)) {
            ...
            for (let action of actions) {
                if (this.MT.isString(action)) {
                    action = { type: 'method', name: action };
                }
                let type = this.MT.extract('type', action, 'method');
                ...
                switch (type) {
                    ...
                    case 'method':
                        let path = action.method || action.name || action.value;
                        let method = this.MT.extract(path);
                        if (method) {
                            promises.push(method(ctx, params));
                        } else {
                            console.error('BOTCMS DO ACTION. METHOD ' + action.name + ' NOT FOUND');
                        }
                        break;
                }
            }
