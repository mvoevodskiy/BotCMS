...
class Scripts {
        ...
        for (const name in script) {
            ...
            if (isC) {
                ...
                for (const key in localAdditional) {
                    if (localAdditional.hasOwnProperty(key) && key !== this.variables.templates) {
            ...
            for (const key in object) {
                if (Object.prototype.hasOwnProperty.call(object, key)) {
                    if (typeof object[key] === 'string') {
                        object[key] = this.copyTemplate(key)
                    }
                }
            }
        }
    ...
    copyTemplate (tmplBinding) {
        if (typeof tmplBinding === 'string' && tmplBinding.startsWith(this.variables.template)) {
            const tmplKey = tmplBinding.substring(this.variables.template.length)
            return tmplKey in this.templates ? mt.copyObject(this.templates[tmplKey]) : null
        }
        return tmplBinding
    }

    handleTemplates (script) {
        if (typeof script === 'object' && script !== null && this.variables.templates in script) {
            for (const tmplKey in script[this.variables.templates]) {
                if (Object.prototype.hasOwnProperty.call(script[this.variables.templates], tmplKey)) {
                    this.templates[tmplKey] = mt.copyObject(script[this.variables.templates][tmplKey])
                }
            }
        }
    }
