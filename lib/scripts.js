...
class Scripts {
    ...
    prepareScripts (parent, script, additional = {}) {
        ...
        for (const name in script) {
            ...
            if (isC) {
                ...
                if (typeof script[name] === 'string') script[name] = { goto: script[name] }
    ...
    checkGotos (scripts = null) {
        // console.log('SCRIPTS CHECK GOTOS STARTED')
        let result = true
        if (scripts === null) scripts = this._scripts.c
        if (typeof scripts === 'object' && scripts !== null) {
            // console.log('SCRIPTS CHECK GOTOS. SCRIPTS KEYS:', Object.keys(scripts))
            for (const key in scripts) {
                if (!Object.prototype.hasOwnProperty.call(scripts, key)) continue
                // console.log('SCRIPTS CHECK GOTOS. KEY:', key, ', TYPEOF SCRIPTS KEY:', typeof scripts[key], ', PATH:', scripts[key]?.path)
                if (typeof scripts[key] === 'object') {
                    // console.log('SCRIPTS CHECK GOTOS. KEYS:', Object.keys(scripts[key]))
                    for (const vld of ['goto', 'validate']) {
                        const gotos = {}
                        if (!(vld in scripts[key])) continue
                        const vldObj = scripts[key][vld]
                        if (typeof vldObj === 'object' && vldObj !== null) {
                            if ('failure' in vldObj) gotos[vld + '.failure'] = vldObj.failure
                            if ('success' in vldObj) gotos[vld + '.success'] = vldObj.success
                            if ('switch' in vldObj) {
                                for (const key in vldObj.switch) {
                                    if (Object.prototype.hasOwnProperty.call(vldObj.switch, key)) gotos[vld + '.switch[' + key + ']'] = vldObj.switch[key]
                                }
                            }
                        } else if (typeof vldObj === 'string') gotos[vld + '.success'] = vldObj
                        for (const localPath in gotos) {
                            if (!Object.prototype.hasOwnProperty.call(gotos, localPath)) continue
                            const target = this.extract(gotos[localPath])
                            if (target === undefined) {
                                console.error('GOTO', gotos[localPath], 'NOT FOUND IN PATH', scripts[key].path, '[', localPath, ']')
                                result = false
                            }
                        }
                    }
                    // if (key === 'c') this.checkGotos(scripts[key])
                    if ('c' in scripts[key]) this.checkGotos(scripts[key].c)
                }
            }
        } else console.log('SCRIPTS CHECK GOTOS. TYPEOF SCRIPTS:', typeof scripts)
        // if (!result) console.error('SCRIPTS CHECK GOTOS FAILED.')
        return result
    }
