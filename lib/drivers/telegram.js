...
class Telegram extends AbstractDriver {
    constructor (BC, params = {}) {
        const attachmentGroupTypes = [
            TYPES.ATTACHMENTS.FILE,
            TYPES.ATTACHMENTS.AUDIO,
            TYPES.ATTACHMENTS.PHOTO,
            TYPES.ATTACHMENTS.VIDEO
        ]
        const defaults = {
            name: 'tg',
            driverName: 'tg',
            ...
            attachmentTypesEqual: {
                [TYPES.ATTACHMENTS.FILE]: 'document',
                // [this.BC.ATTACHMENTS.POLL]: 'poll',
                [TYPES.ATTACHMENTS.PHOTO]: 'photo',
                [TYPES.ATTACHMENTS.AUDIO]: 'audio',
                [TYPES.ATTACHMENTS.VIDEO]: 'video',
                [TYPES.ATTACHMENTS.STICKER]: 'sticker',
                [TYPES.ATTACHMENTS.VIDEONOTE]: 'video_note'
            },
            attachmentGroupTypes,
            attachmentNotGroupTypes: Object.values(TYPES.ATTACHMENTS).filter(type => !Object.values(attachmentGroupTypes).includes(type)),
        ...
        this.ERRORS = {
            ...
            CHAT_ID_IS_EMPTY: 'Bad Request: chat_id is empty',
            ...
            BOT_KICKED_FROM_CHAT_4: 'Forbidden: bot is not a member of the supergroup chat',
            ...
            MESSAGE_TO_EDIT_NOT_FOUND: 'Bad Request: message to edit not found',
            MESSAGE_CAN_NOT_BE_EDITED: 'Bad Request: message can\'t be edited',
            MESSAGE_CAN_NOT_BE_DELETED: 'Bad Request: message can\'t be deleted',
            MESSAGE_CAN_NOT_BE_DELETED_FOR_EVERYONE: 'Bad Request: message can\'t be deleted for everyone',
            QUERY_IS_TOO_OLD: 'Bad Request: query is too old and response timeout expired or query ID is invalid',
            METHOD_IS_AVAILABLE_ONLY_FOR_SUPERGROUPS: 'Bad Request: method is available only for supergroups',
            TOO_MANY_REQUESTS: 'Too Many Requests: retry after ',
            TERMINATED_BY_OTHER: 'Conflict: terminated by other getUpdates request; make sure that only one bot instance is running',
            INVALID_USER_ID: 'Bad Request: invalid user_id specified',
            HAVE_NO_RIGHT_TO_SEND: 'Bad Request: have no rights to send a message'
        }

        // console.log('TG: ');
        // console.log(params);
        this.transport = new Telegraf(params.token, {telegram: this.config.telegram});
        this.Telegram = new TelegramAPI(params.token, this.config.telegram);
        ...
        this.errorHandler = async (error, parcel = {}) => {
            ...
            switch (error.response.description) {
                case this.ERRORS.CHAT_NOT_FOUND:
                    message = { event: this.BC.EVENTS.CHAT_NOT_FOUND, from: { id: peerId }, chat: { id: peerId } }
                    console.error('TG DRIVER. CHAT', peerId, 'NOT FOUND')
                    break

                case this.ERRORS.CHAT_ID_IS_EMPTY:
                    // message = { event: this.BC.EVENTS.CHAT_NOT_FOUND, from: { id: peerId }, chat: { id: peerId } }
                    console.error('TG DRIVER. CHAT ID IS EMPTY')
                    break

                case this.ERRORS.USER_NOT_FOUND:
                    message = { event: this.BC.EVENTS.USER_NOT_FOUND, chat: { id: peerId } }
                ...
                case this.ERRORS.BOT_KICKED_FROM_CHAT_4:
                ...
                case this.ERRORS.MESSAGE_TO_EDIT_NOT_FOUND:
                    console.debug('[TG DRIVER]. MESSAGE TO EDIT NOT FOUND. SENT DATA', error.on)
                    break
                ...
                case this.ERRORS.MESSAGE_CAN_NOT_BE_DELETED:
                    console.error('[TG DRIVER]. MESSAGE CAN NOT BE DELETED. SENT DATA', error.on)
                    break

                case this.ERRORS.MESSAGE_CAN_NOT_BE_DELETED_FOR_EVERYONE:
                    console.error('[TG DRIVER]. MESSAGE CAN NOT BE DELETED FOR EVERYONE. SENT DATA', error.on)
                    break

                case this.ERRORS.TERMINATED_BY_OTHER:
                    console.error('[TG DRIVER]. OTHER BOT INSTANCE WITH CURRENT TOKEN STARTED:', error.on)
                    break

                case this.ERRORS.INVALID_USER_ID:
                    console.error('[TG DRIVER]. SPECIFIED USER ID NOT FOUND. SENT DATA:', error.on)
                    break

                case this.ERRORS.HAVE_NO_RIGHT_TO_SEND:
                    console.error('[TG DRIVER]. HAVE NO RIGHT TO SEND. SENT DATA:', error.on)
                    break
    ...
    async send (Parcel) {
        ...
        // console.log('TG SEND MESSAGE. IN DATA KEYBOAD:');
        // console.dir(Parcel.keyboard, { depth: 5 });
        let parcelMessageSent = false
        let messageIds = [];
        let afterSend = message => {
            // console.log('AFTER SEND. MESSAGE:', message)
            setImmediate(this.messageCallback.bind(this), this, {update: {message: message}});
            return this.config.returnSentMsg ? message : message.message_id;
        };
        let attachmentsHaveCaption = false
        const attachmentGroups = {}
        for (let type of this.config.attachmentGroupTypes) {
            if (!Array.isArray(Parcel.attachments[type]) || Parcel.attachments[type].length === 0) continue
            for (let i in Parcel.attachments[type]) {
                if (!Object.prototype.hasOwnProperty.call(Parcel.attachments[type], i)) continue
                let parcelAttachment = Parcel.attachments[type][i]
                attachmentGroups[type] = attachmentGroups[type] || []
                const prepared = this.prepareMessage(parcelAttachment)
                const attachment = mt.merge(parcelAttachment, {
                    type,
                    media: typeof parcelAttachment === 'object' ? ('id' in parcelAttachment ? parcelAttachment.id : parcelAttachment) : { source: parcelAttachment }
                })
                if (typeof prepared.text === 'string' && prepared.text.length > 0) {
                    attachment.caption = prepared.text
                    attachmentsHaveCaption = true
                    if ('parse_mode' in prepared.extra) attachment.parse_mode = prepared.extra.parse_mode
                }
                attachmentGroups[type].push(attachment)
            }
            // console.log('ATTACHMENT GROUP FOR TYPE: ', type, ':', attachmentGroups[type])
            if (attachmentGroups[type].length >= 2) {
                Parcel.attachments[type] = []
                const mediaPacks = []
                let i = 0
                for (const attachment of attachmentGroups[type]) {
                    mediaPacks[i] = mediaPacks[i] || []
                    mediaPacks[i].push(attachment)
                    if (mediaPacks[i].length === 10) i++
                }
                const lastPack = mediaPacks.length - 1
                if (mediaPacks[lastPack].length === 1) {
                    mediaPacks[lastPack].unshift(mediaPacks[lastPack - 1].pop())
                }
                console.log('MEDIA PACKS:', mediaPacks)
                for (const media of mediaPacks) {
                    const params = {
                        chatId: Parcel.peerId,
                        media,
                    }
                    console.log('SEND MEDIA GROUP PARAMS:', params)
                    const sent = await this.Telegram.sendMediaGroup(Parcel.peerId, media)
                    if (Array.isArray(sent)) {
                        const ids = sent.map(message => afterSend(message))
                        messageIds.push(...ids)
                    }
                }
            }
        ...
        // for (let type of this.config.attachmentNotGroupTypes) {
        // console.log('TG ATTACHMENTS:', Parcel.attachments)
        for (let type of Object.values(TYPES.ATTACHMENTS)) {
            if (typeof Parcel.attachments === 'object' && Parcel.attachments.hasOwnProperty(type)) {
                if (Array.isArray(Parcel.attachments[type])) {
                    attachmentGroups[type] = []
                    for (let attachment of Parcel.attachments[type]) {
                        if (mt.empty(attachment)) continue
                        let file
                        ...
                        let caption = ''
                        let extra = {}
                        let text = attachment.caption
                        if (mt.empty(text) && !parcelMessageSent && !mt.empty(Parcel.message)) {
                            text = Parcel.message
                            parcelMessageSent = true
                        }
                        if (typeof text === 'object') {
                            const prepared = this.prepareMessage(Parcel)
                            // text = prepared.text
                            extra = mt.merge(prepared.extra, this.extraBuild(prepared))
                            extra.caption = this.escapeText(prepared.text, prepared.markup)
                        } else if (typeof text === 'string' && text.length > 0) extra.caption = text
                        extra = this.BC.MT.merge(Parcel.keyboard, this.extraBuild(attachment), extra)
        ...
        if (!parcelMessageSent && Parcel.message !== '') {
            const prepared = this.prepareMessage(Parcel)
            let message = prepared.text
    ...
    restrict (userId, chatId, permissions = {}) {
        console.log('TG RESTRICT. USER ID', userId, 'CHAT ID', chatId, 'PERMISSIONS:', permissions)
        this.Telegram.restrictChatMember(chatId, userId, { permissions: JSON.stringify(permissions) })
    }

    promote (userId, chatId, permissions = {}) {
        this.Telegram.promoteChatMember(chatId, userId, permissions)
    }
    ...
    extraBuild (params, markup = '') {
        ...
        for (let key in this.config.extraParams) {
            if (this.config.extraParams.hasOwnProperty(key) && params.hasOwnProperty(key) /*&& key !== 'markup'*/) {
                let eKey = this.config.extraParams[key];
    ...
    prepareMessage (preparingObject) {
        let text = preparingObject.message || preparingObject.caption || ''
        let extra = {}
        if (typeof text === 'object') {
            extra = this.extraBuild(text);
            text = this.escapeText(text.text, text.markup)
        }
        if (preparingObject.replyMsgId) {
            extra.reply_to_message_id = preparingObject.replyMsgId
        }
        // console.log('TG PREPARE MESSAGE. EXTRA:', extra, 'PREPARING OBJECT:', preparingObject)
        if ('keyboard' in preparingObject) extra = mt.merge(extra, preparingObject.keyboard)
        return { text, extra }
    }

    escapeText (text, markup = '') {
        const symbols = this.config[markup + 'EscapeSymbolsReplace'] || {}
        let re = new RegExp('[' + Object.values(symbols).join('') + ']', 'g');
        // console.log('RE ', re, ' TEXT: ', text);
        return String(text).replace(re, (s) => {
            // console.log('ESCAPE TEXT. S: ', s, ' REPLACE: ', this.config.EscapeSymbolsReplace[s]);
            return symbols[s];
        });
    }
    ...
    async fetchUserInfo (userId, bcContext) {
        ...
        if (userId === this.BC.SELF_SEND || userId === 0 || userId === undefined) {
            ...
            let chatId = typeof bcContext === 'object' && bcContext !== null ? bcContext.Message.chat.id : bcContext;
    ...
    async processUpdate (ctx) {
        return this.messageCallback(this, ctx)
    }
