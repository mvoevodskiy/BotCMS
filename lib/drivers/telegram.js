...
class Telegram {
    constructor (BC, params = {}) {
        ...
        this.ERRORS = {
            ...
            USER_NOT_FOUND: '[Error]: Bad Request: user not found',
    ...
    async messageCallback (t, ctx) {
        ...
        if ('message' in ctx.update) {
            ...
            event = message.event || EVENTS.MESSAGE_NEW;
    ...
    async send (Parcel) {
        ...
        if (Parcel.message !== '') {
            ...
            let id = await this.Telegram[method](...params)
                .then(afterSend)
                .catch((e) => this.sendErrorHandler(e, Parcel));
    ...
    sendErrorHandler (error, parcel = {}) {
        let message = {}
        switch (error.response.description) {
            case this.ERRORS.CHAT_NOT_FOUND:
                console.error('TG DRIVER. CHAT', parcel.peerId, 'NOT FOUND')
                break

            case this.ERRORS.USER_NOT_FOUND:
                message = {
                    event: this.BC.EVENTS.CHAT_NOT_FOUND,
                    // from: { id: parcel.peerId },
                    chat: { id: parcel.peerId }
                }
                break

            case this.ERRORS.USER_BLOCKED:
                message = {
                    event: this.BC.EVENTS.USER_BLOCKED,
                    from: { id: parcel.peerId },
                    chat: { id: parcel.peerId }
                }
                console.error('[TG DRIVER] USER', parcel.peerId, 'IS DEACTIVATED. DO NOTHING')
                break

            case this.ERRORS.BOT_BLOCKED_BY_USER:
                message = {
                    event: this.BC.EVENTS.BOT_BLOCKED_BY_USER,
                    from: { id: parcel.peerId },
                    chat: { id: parcel.peerId }
                }
                break
            ...
            case this.ERRORS.BOT_KICKED_FROM_CHAT_2:
                message = {
                    event: this.BC.EVENTS.BOT_KICKED_FROM_CHAT,
                    // from: { id: parcel.peerId },
                    chat: { id: parcel.peerId }
                }
                break
            ...
            case this.ERRORS.NOT_MODIFIED:
                console.debug('[TG DRIVER]. MESSAGE', parcel.editMsgId, 'IN CHAT', parcel.peerId, 'NOT MODIFIED')
                break

            case this.ERRORS.BOT_CAN_NOT_WRITE_FIRST:
                console.debug('[TG DRIVER]. CONVERSATION WITH USER', parcel.peerId, 'ABSENT. BOT CAN NOT WRITE FIRST. DO NOTHING')
                break
    ...
    remove (peerId, ids) {
        ...
        return Promise.all(promises).catch((e) => this.sendErrorHandler(e))
    }
