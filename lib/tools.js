...
class Tools {
    ...
    async validate (ctx, gotos) {
        ...
        if (!this.MT.empty(validator)) {
            switch (validator) {
                ...
                //
                case 'text':
                    if (typeof ctx.msg === 'string' && ctx.msg.length) {
                        if (result && validatorParams.min !== undefined) {
                            result = ctx.msg.length >= validatorParams.min
                        }
                        if (result && validatorParams.max !== undefined) {
                            result = ctx.msg.length <= validatorParams.max
                        }
                    } else result = false
                    // console.log('BOTCMS VALIDATE. TEXT LENGTH:', ctx.msg.length, 'PARAMS:', validatorParams, 'RESULT:', result)
                    break
                ...
                case 'number':
                    if (!this.MT.empty(ctx)) {
                        ...
                        if (result) ctx.msg = String(number)
                    }
                    break;
    ...
    removeUsernameFromCommand (msg, username) {
        return msg.replace(new RegExp('(?<=^\\/\\w+)' + username + '(?=($|( .*)))'), '')
    }
