...
class Logger {
  ...
  async handle (dto) {
    ...
    const normStatData = this.MT.mergeRecursive(this.emptyNormLogData, {
      ...
      path: dto.path || (dto.ctx ? dto.ctx.session.step.path : ''),
      user: this.MT.extract('ctx.Message.sender', dto, { id: this.MT.extract('methodParams.peerId', dto, 0) }),
      chat: this.MT.extract('ctx.Message.chat', dto, { id: this.MT.extract('methodParams.peerId', dto, 0) }),
      language: dto.ctx ? dto.ctx.language : this.BC.config.language,
      sid: dto.ctx ? dto.ctx.getUid() : '',
      requestStartTime: dto.ctx ? dto.ctx.getStartTime() : Date.now(),
      additional: dto.additional || {}
    })
    // console.log('BOTCMS LOGGER. EMPTY NORMALIZED LOG DATA:', this.emptyNormLogData)
    const promises = []
